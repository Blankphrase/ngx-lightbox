...
class NgxHotkeysService {
    ...
    register(hotkey, unpausing = false) {
        let /** @type {?} */ hotkeys = [].concat(hotkey);
        if (unpausing) {
    ...
    unregister(hotkey, pausing = false) {
        const /** @type {?} */ hotkeys = [].concat(hotkey);
        hotkeys.forEach(h => {
    ...
    bindToMoustrap(hotkey) {
        this._mousetrapInstance.bind(hotkey.combo, (event, combo) => {
            ...
            if (event) {
                const /** @type {?} */ target = /** @type {?} */ ((event.target || event.srcElement)); // srcElement is IE only
                const /** @type {?} */ nodeName = target.nodeName.toUpperCase();
