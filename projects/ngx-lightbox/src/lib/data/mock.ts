import {GalleryAlbum} from '../interfaces';

export const _albums: GalleryAlbum[] = [
  {
    title: 'MINIMALISM',
    folderPath: 'minimalism',
    files: [
      {
        title: 'Intro',
        fileName: 'https://images.unsplash.com/photo-1519996121844-d53802c2db84?ixlib=rb-0.3.5&s=d1dee2a9fa3c638b22032ccc870d6a50&auto=format&fit=crop&w=634&q=80'
      },
      {
        title: '1',
        fileName: 'https://images.unsplash.com/photo-1520013817300-1f4c1cb245ef?ixlib=rb-0.3.5&s=8ca7af7198a3db03a788fa403f253ad0&auto=format&fit=crop&w=1387&q=80'
      },
      {
        title: '2',
        fileName: 'https://images.unsplash.com/photo-1520079227293-0888e274dd20?ixlib=rb-0.3.5&s=b290bf50e442176e8d5b5d14e2d63747&auto=format&fit=crop&w=700&q=80'
      }
    ]
  },
  {
    title: 'Adventure',
    folderPath: 'adventure',
    files: [
      {
        title: 'Jump',
        fileName: 'https://images.unsplash.com/photo-1492567291473-fe3dfc175b45?ixlib=rb-0.3.5&s=6efb2d1282b5122a35a694dbcfd356f7&auto=format&fit=crop&w=631&q=80'
      }
    ]
  },
  {
    title: 'Simple but Complex',
    folderPath: 'simple_but_complex',
    files: [
      {
        title: 'Volcano pattern',
        fileName: 'https://images.unsplash.com/photo-1519986988383-5b8327613e72?ixlib=rb-0.3.5&s=9ce53eaead8e6295344399eec9f8ac00&auto=format&fit=crop&w=641&q=80'
      }
    ]
  }
];
